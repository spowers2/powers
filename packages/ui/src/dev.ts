/**
 * Lightweight development warnings for common Power UI mistakes.
  ...
  queueMicrotask(() => {
    try {
      ...
      if (!accent) {
        devWarnOnce(
          "theme-css",
          'Missing design tokens. Import once at the app root: import "@power-ui/ui/theme.css"; then createTheme().bind().',
        );
      }
    } catch {
      /* ignore */
...
/** Enable/disable all Power UI dev warnings. */
