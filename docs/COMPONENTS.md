...
Power UI is designed so **new primitives are cheap** and **brand overrides don’t require forking**.
...
import { cx } from "@power-ui/ui";
import { createStyleSheet } from "@power-ui/ui";
...
```tsx
import { styleVars, Card } from "@power-ui/ui";
...
| Motion | `Transition` (+ `@power-ui/animate` springs) |
| Authoring | `createStyleSheet` `styleVars` `trapFocus` `cx` |

React ships **no** UI kit. Bootstrap covers forms/nav/overlay well — Power UI adds **product primitives** (Stat, Timeline, Steps, Empty, Command, Combobox, ToggleGroup, Table, List, Chip, NumberInput, AspectRatio, ScrollArea, Collapse, Transition) on a fine-grained reactive core.
