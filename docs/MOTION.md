...
| Number / progress tween | `@lab206/animate` `animate` / `spring` on a **signal** |
| Complex timelines, scrub, SVG | Optional peer **GSAP** via `@lab206/animate/gsap` |
...
import { Transition, Button, Card } from "@lab206/ui";
...
```ts
import { MOTION_PRESETS } from "@lab206/ui";
...
Number signals / springs: `@lab206/animate` (`animate`, `spring`, `cancel`).  
